...
RUN cd /tmp && \
	...
	apt purge -y php8* && \
	apt-get -y --no-install-recommends install curl wget git sudo cron locales vim supervisor build-essential autoconf php-pear php${PHP_RELEASE}-dev zlibc zlib1g zlib1g-dev && \
...
RUN cd /tmp && \
	...
	pecl install grpc && phpenmod grpc && \
	echo "extension=grpc.so" > /etc/php/${PHP_RELEASE}/mods-available/grpc.ini && \
	phpenmod grpc
